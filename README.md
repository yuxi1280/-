# 基于HC32F460的充电桩控制系统

## 📖 项目介绍

这是一个专为新能源充电桩设计的完整控制系统，基于HC32F460(32位ARM Cortex-M4)微控制器开发。该项目提供了一套全面的硬件驱动和应用层代码，支持寄存器级别的直接编辑，兼容VSCode开发环境，可进行快速程序编译和在线烧录。

系统采用模块化设计，各功能独立可配置，适用于多种充电桩硬件平台，具有较高的可移植性和可扩展性。

## ✨ 主要特性

- 🔧 **完整的HC32F460驱动库** - 提供所有外设的底层驱动支持
- 📝 **寄存器级编辑** - 支持直接修改控制寄存器，方便调试和优化
- 💻 **VSCode原生支持** - 完全兼容VSCode开发环境，无需重型IDE
- 📥 **在线程序下载** - 支持通过串口/USB的快速程序烧录
- 🔌 **模块化架构** - 各功能模块独立设计，便于集成和维护
- 🚀 **高性能** - 充分利用Cortex-M4高性能特性，支持实时处理

## 📋 项目结构详解

```
.
├── src/                          # 源代码文件夹
│   ├── main.c                    # 主程序入口
│   ├── uart.c                    # UART串口驱动│   ├── uart.c                    # UART serial port driver│   ├── uart.c                    # UART 串口驱动程序
│   ├── rs485.c                   # RS485通信驱动│   ├── rs485.c                   # RS485 communication driver│   ├── rs485.c                   # RS485 通信驱动程序
│   ├── spi.c                     # SPI接口驱动│   ├── spi.c                     # SPI interface driver
│   ├── crc16_modbus.c            # Modbus CRC校验算法│   ├── crc16_modbus.c            # Modbus CRC check algorithm
│   ├── encoder.c                 # 编码器驱动│   ├── encoder.c                 # Encoder driver
│   ├── pid_pos.c                 # 位置PID控制│   ├── pid_pos.c                 # Position PID control
│   ├── pid_speed.c               # 速度PID控制│   ├── pid_speed.c               # Speed PID control
│   ├── moto.c                    # 电机控制│   ├── moto.c                    # Motor control
│   ├── flash.c                   # Flash存储管理│   ├── flash.c                   # Flash storage management
│   ├── at24c02.c                 # 24C02 EEPROM驱动
│   ├── ws2812.c                  # WS2812 RGB灯控制│   ├── ws2812.c                  # WS2812 RGB LED control
│   ├── soft_timer.c              # 软定时器│   ├── soft_timer.c              # Soft Timer
│   ├── ir_comm.c                 # 红外通信│   ├── ir_comm.c                 # Infrared communication
│   ├── relay_ctrl.c              # 继电器控制│   ├── relay_ctrl.c              # Relay control
│   ├── msg.c                     # 消息队列系统
│   ├── doraemon_pack.c           # 数据打包协议│   ├── doraemon_pack.c           # Data packaging protocol
│   ├── io.c                      # GPIO输入输出│   ├── io.c                      # GPIO input and output
│   └── mix_std_lib.c             # 混合标准库函数│   └── mix_std_lib.c             # Mixed standard library functions
│
├── inc/                          # 头文件夹
│   ├── uart.h, rs485.h, ...      # 对应的头文件定义
│
├── sys/                          # 系统文件
│   ├── startup_hc32f460.s        # ARM汇编启动代码│   ├── startup_hc32f460.s        # ARM assembly startup code
│   └── cmsis/                    # CMSIS标准库│   └── cmsis/                    # CMSIS standard library
│       └── core/                 # ARM Cortex-M4核心头文件│       └── core/                 # ARM Cortex-M4 core header files
│
├── build/                        # 编译配置├── build/                        # Build configuration
│   └── makefile                  # GNU Make编译脚本│   └── makefile                  # GNU Make build script
│
├── obj/                          # 编译生成的目标文件
│   └── EPC_DCBM.dis              # 反汇编文件
│
├── LICENSE                       # 许可证文件
└── README.md                     # 本文件
```

## 🚀 快速开始

### 环境要求

| 工具 | 版本 | 说明 |
|------|------|------|
| VSCode | 最新版 | 代码编辑器 |
| GCC ARM | arm-none-eabi-gcc | ARM交叉编译工具链 |
| Make | 3.81+ | 编译构建工具 |
| OpenOCD | 0.10+ | 烧录和调试工具 |

### 安装步骤

#### 1. 配置编译工具链

**Windows系统:**
```bash
# 下载并安装 ARM GCC
# https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/gnu-rm

# 下载并安装 Make
# http://gnuwin32.sourceforge.net/packages/make.htm

# 验证安装
arm-none-eabi-gcc --version
make --version
```

**Linux/Mac系统:**
```bash
# Ubuntu/Debian
sudo apt-get install build-essential gcc-arm-none-eabi

# macOS (使用 Homebrew)
brew install arm-none-eabi-gcc
```

#### 2. VSCode扩展安装

推荐安装以下VSCode扩展以提升开发体验：
- **Cortex-Debug** - ARM调试支持
- **C/C++ IntelliSense** - 代码智能补全
- **Makefile Tools** - Makefile支持
- **Serial Monitor** - 串口监视器

#### 3. 克隆项目

```bash
git clone https://github.com/yourusername/hc32f460-charger.git
cd hc32f460-charger
```

### 编译与烧录

#### 编译项目

```bash
cd build
make clean          # 清除之前的编译产物
make all            # 编译全部
# 生成的bin文件在 obj/ 目录下
```

#### 烧录程序

**使用OpenOCD:**
```bash
openocd -f interface/stlink-v2.cfg -f target/stm32f4x.cfg
# 在另一个终端中使用GDB烧录
arm-none-eabi-gdb obj/EPC_DCBM.elf
```

**使用串口烧录工具:**
```bash
# 使用专用烧录软件，选择COM口和波特率(通常为115200)
# 将程序烧录到HC32F460
```

## 🔌 主要功能模块详解

### 通信模块

| 模块 | 文件 | 功能说明 |
|------|------|--------|
| **UART** | uart.c/h | 异步串行通信，波特率可配，支持中断模式 |
| **RS485** | rs485.c/h | 模总线协议实现，最多支持32个节点 |
| **IR** | ir_comm.c/h | 红外遥控通信，编码解码 |

### 控制模块

| 模块 | 文件 | 功能说明 |
|------|------|--------|
| **PID位置控制** | pid_pos.c/h | 位置闭环控制算法，参数可动态调整 |
| **PID速度控制** | pid_speed.c/h | 速度闭环控制，采样频率1kHz |
| **电机驱动** | moto.c/h | PWM调速，方向控制，过流保护 |
| **继电器控制** | relay_ctrl.c/h | 多路继电器控制，带状态反馈 |

### 存储模块

| 模块 | 文件 | 功能说明 |
|------|------|--------|
| **Flash** | flash.c/h | 内部Flash读写，支持扇区擦除 |
| **EEPROM** | at24c02.c/h | 24C02外部存储，I2C接口 |

### 其他功能

| 模块 | 文件 | 功能说明 |
|------|------|--------|
| **编码器** | encoder.c/h | 旋转编码器计数，支持正反向 |
| **RGB灯** | ws2812.c/h | WS2812B彩色灯珠控制，支持流水/渐变效果 |
| **软定时器** | soft_timer.c/h | 非实时定时器，用于定时任务 |
| **CRC校验** | crc16_modbus.c/h | Modbus标准CRC16校验 |
| **消息队列** | msg.c/h | 任务间消息传递机制 |
| **数据打包** | doraemon_pack.c/h | 自定义通信协议实现 |
| **GPIO** | io.c/h | 通用数字I/O操作 |

## 📝 使用示例

### 示例1: 基础串口通信

```c   ' ' ' c
#include "uart.h"

int main(void) {
    uart_init(UART1, 115200);  // 初始化UART1，波特率115200
    
    uart_send_string(UART1, "Hello World!\n");
    
    while(1) {
        if(uart_available(UART1)) {
            char c = uart_recv_char(UART1);
            uart_send_char(UART1, c);  // 回显字符
        }
    }
}
```

### 示例2: PID速度控制

```c
#include "pid_speed.h"
#include "encoder.h"

int main(void) {
    encoder_init();
    pid_speed_init(1.0, 0.5, 0.1);  // P, I, D参数
    
    uint16_t target_speed = 1000;  // 目标速度
    
    while(1) {
        uint16_t current_speed = encoder_get_speed();
        uint16_t pwm_val = pid_speed_calc(target_speed, current_speed);
        set_motor_pwm(pwm_val);
    }
}
```

### 示例3: RS485通信

```c
#include "rs485.h"
#include "crc16_modbus.h"

int main(void) {
    rs485_init(115200);
    
    uint8_t tx_buf[256];
    uint8_t rx_buf[256];
    
    // 构建Modbus RTU报文
    tx_buf[0] = 0x01;           // 从机地址
    tx_buf[1] = 0x03;           // 读保持寄存器功能码
    tx_buf[2] = 0x00;           // 起始地址高字节
    tx_buf[3] = 0x00;           // 起始地址低字节
    tx_buf[4] = 0x00;           // 寄存器个数高字节
    tx_buf[5] = 0x02;           // 寄存器个数低字节
    
    // 计算CRC校验码
    uint16_t crc = crc16_modbus(tx_buf, 6);
    tx_buf[6] = crc & 0xFF;     // CRC低字节
    tx_buf[7] = (crc >> 8) & 0xFF;  // CRC高字节
    
    rs485_send(tx_buf, 8);
    rs485_recv(rx_buf, 256);
    
    return 0;
}
```

## 🔧 配置说明

主要配置文件位于 `inc/` 目录下，常见配置项：

```c
// uart.h - 串口配置
#define UART1_BAUDRATE      115200
#define UART1_BUFFER_SIZE   256

// pid_speed.h - PID参数
#define PID_KP              1.0
#define PID_KI              0.5
#define PID_KD              0.1

// moto.h - 电机配置
#define MOTOR_PWM_FREQ      20000    // PWM频率 20kHz
#define MOTOR_PWM_MAX       1000     // PWM最大值
```

## 🐛 调试方法

### 使用OpenOCD + GDB调试

```bash
# 启动OpenOCD守护进程
openocd -f interface/stlink-v2.cfg -f target/stm32f4x.cfg &

# 使用GDB连接
arm-none-eabi-gdb obj/EPC_DCBM.elf
(gdb) target remote localhost:3333
(gdb) load                    # 加载程序
(gdb) break main              # 在main处设置断点
(gdb) continue                # 继续运行
(gdb) step                    # 单步执行
```

### 使用串口监视器查看日志

在VSCode中使用Serial Monitor扩展，配置COM口和波特率，实时查看串口输出。

## 📊 性能指标

| 指标 | 数值 |
|------|------|
| 主频 | 200 MHz |
| 内存 | 256 KB SRAM |
| Flash | 512 KB |
| GPIO | 100+ |
| UART | 4个 |
| SPI | 3个 |
| I2C | 2个 |

## 📄 许可证

本项目采用 **MIT许可证** 开源发布，详见 [LICENSE](LICENSE) 文件。

## 👥 贡献指南

欢迎提交Issue和Pull Request！

### 如何贡献

1. Fork本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启Pull Request

### 代码规范

- 使用GNU风格缩进(4空格)
- 函数和变量采用蛇形命名法: `function_name`, `variable_name`
- 为复杂逻辑添加注释
- 新功能需附带单元测试


## 🙏 致谢

感谢所有贡献者和使用者的支持！

---

**最后更新**: 2026年1月22日
